...
## 3. Clean bed graph
mkdir -p ${new_path}/${sig}  

names=`ls ${old_path}/${sig} | cut -f 1 -d '.'`

for name in ${names};do
   bedtools sort -i ${old_path}/${sig}/${name}.bed > ${new_path}/${sig}/${name}.2.bed
   bedtools merge -i ${new_path}/${sig}/${name}.2.bed -c 4 -o max > ${new_path}/${sig}/${name}.bed
   rm ${new_path}/${sig}/${name}.2.bed
done


## 4. bedgraph to annot (EAS)
...
for bedline in ${bedlines};do
 ...
 python code_R/sclinker/bedgraph_to_annot_EAS.py \
  --bedname ${bedname} \
  --bedfile_path ${old_path}/${sig} \
  --bimfile_path ${bim_path} \
  --annot_path ${new_path}/${sig}/${bedname}

done


## bedgraph_to_annot_EAS.py
...
for numchr in range(1, 23, 1):
    bimfile = bimfile_path + "/" + "1000G.EAS.QC." + str(numchr) + ".bim"
